...
from database.models import User, Conversation, UserStats, Gender, CommunicationStyle, CREATE_TABLES_SQL
    ...
    async def create_user(self, user: User) -> User:
        ...
        async with self.pool.acquire() as conn:
            await conn.execute(
                ...
                user.gender.value, user.bot_gender.value, user.communication_style.value,
                user.consent_given, user.stop_words
            )
    ...
    async def save_conversation(self, conversation: Conversation) -> Conversation:
        ...
        async with self.pool.acquire() as conn:
            await conn.execute(
                ...
                VALUES ($1, $2, $3, $4, $5)
                """,
                conversation.user_id, conversation.message, conversation.bot_response,
                conversation.communication_style.value, conversation.tokens_used
            )
            ...
            await conn.execute(
                ...
                WHERE user_id = $1
                """,
                conversation.user_id, conversation.tokens_used
            )
