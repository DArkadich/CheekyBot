...
class OpenAIService:
    ...
    def _generate_cache_key(self, message: str, style: CommunicationStyle, 
                           user_gender: Gender, bot_gender: Gender) -> str:
        """Генерация ключа кеша для запроса"""
        content = f"{message}:{style.value}:{user_gender.value}:{bot_gender.value}"
        return hashlib.md5(content.encode()).hexdigest()
    
    def _get_style_prompt(self, style: CommunicationStyle, user_gender: Gender, 
                         bot_gender: Gender) -> str:
    ...
    async def generate_response(self, message: str, style: CommunicationStyle,
                              user_gender: Gender, bot_gender: Gender,
                              stop_words: List[str] = None) -> Optional[str]:
    ...
    async def generate_roleplay_scenario(self, scenario_type: str, 
                                       user_gender: Gender, bot_gender: Gender) -> Optional[str]:
        ...
        try:
            ...
            scenario_description = scenarios.get(scenario_type, "Романтическое свидание")
