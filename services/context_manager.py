...
class ContextManager:
    ...
    async def get_context(self, user_id: int, max_messages: int = 10) -> List[Dict[str, str]]:
    ...
    async def _update_summary(self, user_id: int, context: List[Dict[str, Any]]) -> None:
    ...
    async def get_user_preferences(self, user_id: int) -> Dict[str, str]:
        ...
        elif any(word in all_content.lower() for word in ["романтика", "любовь", "нежность"]):
            preferences["communication_style"] = "romantic"
        elif any(word in all_content.lower() for word in ["страсть", "эмоции", "чувства"]):
